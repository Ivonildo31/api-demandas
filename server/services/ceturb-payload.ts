...
export let ceturbPayload = {
  getEmailBody ( demand: IDemand ) {
    ...
    let mailBody = `Data de envio: ${moment( demand.createdAt ).local().format( 'DD/MM/YYYY HH:mm:ss' )}\n`
    ...
    mailBody += payload.date ? `hora: ${moment( demand.createdAt.substring( 0, 11 ) + payload.date.substring( 11 ) ).local().format( 'HH:mm' )}\n` : ''
